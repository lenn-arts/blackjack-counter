...
static void write_value(int val[], int max_addr){
    ...
    for (addr = 0; addr < max_addr; addr = addr + 1){
        // arr[addr]
       iowrite32(val[addr], dev.virtbase + addr); // write 8 bits
...
static int* read_value(int addr, int max_addr){
    ...
    for (addr_local = 0; addr_local < max_addr-addr; addr_local = addr_local + 1){
        *(out_ptr+addr_local) = ioread32(dev.virtbase+addr+addr_local);
        pr_info("Kread_value: from %d read %d, %d", addr_local, *(out_ptr+addr_local), ioread32(dev.virtbase+addr+addr_local));
    }
