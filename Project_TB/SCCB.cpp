...
int main(int argc, const char ** argv, const char ** env) {
  ...
  VSCCB * dut = new VSCCB;  // Instantiate the collatz module
  ...
  std::cout << dut->start; // Print the starting value of the sequence
  ...
  for (time = 0 ; time < 10000 ; time += 10) {
    ...
    if (dut->clk && !last_clk && !dut->start) {
      if (time >= 60) std::cout << ' ' << dut->start; // Print the next value
